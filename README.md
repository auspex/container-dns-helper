...
This approach uses `python-sdbus-networkmanager` to have the host's NetworkManager create a new `macvlan` interface as a child of your primary Internet-facing ethernet NIC, for any container with a `label` of `dhcp=true`. A `macvlan` interface has a new MAC address, different from anything existing on your host. Therefore, when NetworkManager brings it up, it will get assigned a new IP—even though traffic will actually flow through the parent interface.
...
* `/var/run/dbus` - so that `python-sdbus-networkmanager` can communicate with the host's NetworkManager
...
* docker.io/library/python:3.13.3-slim
